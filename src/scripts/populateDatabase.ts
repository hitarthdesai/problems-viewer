...
import {
	attachProblemAndCompany,
	createDifficultyNodes,
	createProblemNodeAndRelationships,
} from '@/utils/database/problem';
...
export async function populateDatabase() {
	...
	try {
		...
		await session.run(
			`CREATE FULLTEXT INDEX companiesProblemsAndTopics FOR (n:Company|Problem|Topic) ON EACH [n.name, n.slug, n.title] OPTIONS {
			indexConfig: {
